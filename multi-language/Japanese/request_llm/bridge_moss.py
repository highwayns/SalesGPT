...
load_message = "MOSSがまだロードされていません，読み込みには時間がかかります。注意，依存する`config.py`テキストの翻訳，原始文本（CPU）原始文本（GPU），おそらく低スペックのコンピューターがフリーズする可能性があります"

#################################################################################
class GetGLMHandle(Process):
    def __init__(self): # メインプロセスの実行
    ...
    def check_dependency(self): # メインプロセスの実行
        try:
            import datasets, os
            ...
            self.info = """
            MOSSの依存関係がしない足しています，MOSSを使用するする場合，基本的なpipの依存関係以外，您还需要运行`pip install -r request_llm/requirements_moss.txt`と`git clone https://github.com/OpenLMLab/MOSS.git request_llm/moss`MOSSの依存関係をインストールする。
            """
            self.success = False
        return self.success
    ...
    def moss_init(self): # 子プロセスの実行
        # 子プロセスの実行
        # このコードの出典はhttps://github.com/OpenLMLab/MOSS/blob/main/moss_cli_demo.py
        import argparse
        import os
        import platform
        import warnings
    ...
    def run(self): # 子プロセスの実行
        ...
        try:
            ...
            raise RuntimeError("MOSSパラメータを正常にロードできません！")
    ...
    def stream_chat(self, **kwargs): # メインプロセスの実行
        # メインプロセスの実行
...
def predict(inputs, llm_kwargs, plugin_kwargs, chatbot, history=[], system_prompt='', stream = True, additional_fn=None):
    ...
    if moss_handle is None:
        ...
        response = "[Local Message]: MOSSの応テキストの翻訳を待っています ..."
    ...
    # 要約出力
    if response == "[Local Message]: MOSSの応テキストの翻訳を待っています ...":
        response = "[Local Message]: MOSSの応テキストの翻訳が異常です ..."
