...
"""
    このファイルには主に2つの関数が含まれています，すべてのLLMの共通インターフェースです，テキストの翻訳，複数のモデルの並列処理などの詳細を処理します

    マルチスレッドの機能を持たない関数：通常の会話時に使用するする，テキストの翻訳，マルチスレッドはできません
    1. predict(...)

    原始文本：関数プラグインで呼び出されます，テキストの翻訳
...
class LazyloadTiktoken(object):
    ...
    def get_encoder(model):
        print('トークナイザを読み込んでいます，初めて実行する場合，パラメータのダウンロードに少し時間がかかる場合があります')
...
try:
    ...
    if API_URL != "https://api.openai.com/v1/chat/completions": 
        ...
        print("警告！API_URLの設定オプションは廃止されます，API_URL_REDIRECTの設定に変更してください")
except:
    pass
# 新しいバージョンの設定
...
if "qwen" in AVAIL_LLM_MODELS:
    try:
        ...
        print(trimmed_format_exc())
if "chatgpt_website" in AVAIL_LLM_MODELS:   # いくつかの逆工学httpsに接続する://github.com/acheong08/ChatGPT-to-API/
    try:
        ...
        print(trimmed_format_exc())
if "spark" in AVAIL_LLM_MODELS:   # 讯飞星火認知大モデル
    try:
        ...
        print(trimmed_format_exc())
if "sparkv2" in AVAIL_LLM_MODELS:   # 讯飞星火認知大モデル
    try:
        from .bridge_spark import predict_no_ui_long_connection as spark_noui
        from .bridge_spark import predict as spark_ui
        model_info.update({
...
def predict_no_ui_long_connection(inputs, llm_kwargs, history, sys_prompt, observe_window, console_slience=False):
    """
    LLMに送信する，返信待ち，一度に完了する，中間プロセスを表示しない。ただし、内部ではストリームを使用するして途中でネットワーク接続が切断されるのを防ぐ。
    inputs：
        今回の問い合わせの入力です
    sys_prompt:
        システムの静的プロンプト
    ...
    if '&' not in model:
        ...
        # 1つの大規模言語モデルのみを問い合わせる場合：
        method = model_info[model]["fn_without_ui"]
        return method(inputs, llm_kwargs, history, sys_prompt, observe_window, console_slience)
    else:

        # 複数の大規模言語モデルに同時に問い合わせる場合，これは少し冗長です，テキストの翻訳，このelseブランチを読む必要はありません
        ...
        def mutex_manager(window_mutex, observe_window):
            while True:
                ...
                chat_string = []
                for i in range(n_model):
                    chat_string.append( f"【{str(models[i])} 言う】: <font color=\"{colors[i]}\"> {window_mutex[i][0]} </font>" )
                res = '<br/><br/>\n\n---\n\n'.join(chat_string)
                # # # # # # # # # # #
        ...
        for i, future in enumerate(futures):  # wait and get
            return_string_collect.append( f"【{str(models[i])} 言う】: <font color=\"{colors[i]}\"> {future.result()} </font>" )
...
def predict(inputs, llm_kwargs, *args, **kwargs):
    ...
    method = model_info[llm_kwargs['llm_model']]["fn_with_ui"]  # もしエラーが発生した場合，configのAVAIL_LLM_MODELSオプションを確認してください
